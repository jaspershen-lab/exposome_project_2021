...
####internal_ome
load(
  "3_data_analysis/mediation_analysis/data_preparation/internal_ome_expression_data"
)
load("3_data_analysis/mediation_analysis/data_preparation/internal_ome_sample_info")
load(
  "3_data_analysis/mediation_analysis/data_preparation/internal_ome_variable_info"
)


head(rownames(exposome_expression_data))
...
behavior_glm_p =
  ...
  purrr::map(function(x) {
    temp_data =
      data.frame(x = x, exposome_sample_info)
...
exposome_phenotype_glm =
  data.frame(behavior_glm_p,
             Body.mass.index.z.score_glm_p,
             Intelligence.quotient_glm_p) %>%
  tibble::rownames_to_column(var = "variable_id")

exposome_phenotype_glm$behavior_glm_p.adjust = p.adjust(exposome_phenotype_glm$behavior_glm_p, method = "BH")
exposome_phenotype_glm$Body.mass.index.z.score_glm_p.adjust = p.adjust(exposome_phenotype_glm$Body.mass.index.z.score_glm_p,
                                                                       method = "BH")
