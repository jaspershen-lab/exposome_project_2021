...
###child exposome air
load(
  "3_data_analysis/5_exposome_air_data_analysis/data_preparation/expression_data"
)
...
transcriptome_variable_info = 
transcriptome_variable_info %>% 
  dplyr::distinct(GeneSymbol_Affy, .keep_all = TRUE)

transcriptome_expression_data = 
transcriptome_expression_data[transcriptome_variable_info$variable_id,]
...
transcriptome_variable_info$mol_name = transcriptome_variable_info$GeneSymbol_Affy
proteome_variable_info$class = "proteome"
proteome_variable_info$mol_name = proteome_variable_info$Gene_Symbol
serum_metabolome_variable_info$class = "serum_metabolome"
serum_metabolome_variable_info$mol_name = serum_metabolome_variable_info$var
urine_metabolome_variable_info$class = "urine_metabolome"
urine_metabolome_variable_info$mol_name = urine_metabolome_variable_info$var
...
sum(duplicated(internal_ome_variable_info$mol_name))

internal_ome_sample_info = 
  rbind(
    transcriptome_sample_info[,c("sample_id", "subject_id", "e3_sex", "age_sample_years")]
  )
  
dim(internal_ome_sample_info)
dim(internal_ome_expression_data)

colnames(internal_ome_expression_data) == internal_ome_sample_info$sample_id


setwd(r4projects::get_project_wd())
dir.create("3_data_analysis/mediation_analysis/data_preparation")
setwd("3_data_analysis/mediation_analysis/data_preparation")
...
save(internal_ome_expression_data, file = "internal_ome_expression_data")
save(internal_ome_sample_info, file = "internal_ome_sample_info")
save(internal_ome_variable_info, file = "internal_ome_variable_info")
